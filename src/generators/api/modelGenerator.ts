...
function generateUniqueClassName(base: string): string {
 let name = capitalize(base) + 'Model';
 let i = 1;
 while (generatedClassNames.has(name)) {
   name = capitalize(base) + i + 'Model';
   i++;
 }
...
export function generateClass(
 ...
 function buildClassCode(currentName: string, props: Record<string, any>): string {
   const constructorLines = Object.keys(props).sort().map((prop) => {
     const value = props[prop];
     if (value?.type === 'array') return `this.${prop} = [];`;
...
set${capitalize(prop)}(items) {
 ...
 this.${prop} = items.map(item => {
   ...
   return item;
     ...
     if (propType === 'object') {
       const classRef = capitalize(prop) + 'Model';
       return `
get${capitalize(prop)}() {
 return this.${prop};
}
...
`.trim();
 ...
 function traverse(
   name: string,
   body: any
 ): { main: string; inner: Record<string, string> } {

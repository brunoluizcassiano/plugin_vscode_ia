...
import { generateRequestBodyClasses, generateClassNameFromPath } from './modelGenerator';
...
*/
export async function generateModelFromOpenAPI(openApiPath: string, endpoint: string | undefined, generatePathAppDriver: string): Promise<void> {
  ...
  if (endpoint) {
    ...
    if (methods.length === 0) {
      ...
      await generateRequestBodyClasses(endpoint, methods, paths[endpoint], openApiDoc, generatePathAppDriver);
    }
  } else {
    ...
    for (const endpointPath of Object.keys(paths)) {
      ...
      await generateRequestBodyClasses(endpointPath, methods, paths[endpointPath], openApiDoc, generatePathAppDriver);
    }
  }

...
function getZephyrViewContent() {
  ...
  function filterProjetos(){
    ...
    projectFlow.select.innerHTML = '<option value="">Selecione...</option>' +
      filtered.map(p => '<option value="'+(p.id || p.key)+'" '+(((p.id||p.key)===cur)?'selected':'')+'>'+ (p.name || p.key || '') + (p.key ? ' ('+p.key+')' : '') +'</option>').join('');
    projectFlow.btnContinue.disabled = !projectFlow.select.value;
  }
  function renderFolderTree(list){
    ...
    const mk = function(n){
      ...
      const cb = document.createElement('input'); cb.type='checkbox'; cb.dataset.id = n.id;
      const sp = document.createElement('span'); sp.textContent = n.name || '(sem nome)';
      li.appendChild(cb); li.appendChild(sp);
      if (n.children && n.children.length){
        const ul = document.createElement('ul'); ul.className='tree';
        n.children.forEach(c => ul.appendChild(mk(c)));
        li.appendChild(ul);
      }
      return li;
    };
    const frag = document.createDocumentFragment();
    arr.forEach(n => frag.appendChild(mk(n)));
    projectFlow.folderTree.appendChild(frag);
  }
  ...
  function getSelectedFolderIds(){
    return Array.from(projectFlow.folderTree.querySelectorAll('input[type="checkbox"]:checked')).map(i => i.dataset.id);
  }
  ...
  function updateApplyEnabled(){
    projectFlow.btnApply.disabled = !(getSelectedFolderIds().length || getSelectedFilterIds().length);
  }

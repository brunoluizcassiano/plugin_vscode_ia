...
function getZephyrViewContent() {
  ...
  /* ===== estilos mínimos p/ Pastas & Filtros ===== */
  ...
  /* ===================== [TREEVIEW] Estilo de árvore tipo IDE ===================== */
  ...
  .tree-item.expanded::before { transform: rotate(90deg); }
  .tree-item:hover { background: rgba(255,255,255,0.06); }
  .tree-item.selected { background: rgba(79,195,247,0.18); }
  .nested {
    display: none;
    margin-left: 12px;
  ...
  <div id="projectFlow" style="display:none; margin-bottom:1rem;">
    ...
    <div class="row">
      ...
      <!-- (mantido: sem barra de busca/botões extras) -->
      <div class="col" style="display:flex; gap:.5rem; align-items:center; visibility:hidden;">
        <button id="btnReloadProjects" style="display:none;">Recarregar</button>
        <button id="btnContinueProject" style="display:none;" disabled>Continuar</button>
      </div>
    </div>
    ...
    <div id="projectStructure" style="display:none; margin-top:1rem;">
      ...
      <div style="display:flex; gap:.5rem; justify-content:flex-start; margin-top:.75rem;">
        <!-- removido "Voltar" a seu pedido -->
  ...
  /* ===== refs do fluxo por projeto ===== */
  const projectFlow = {
    root: document.getElementById('projectFlow'),
    select: document.getElementById('projectSelect'),
    search: document.getElementById('projectSearch'),           // pode não existir (foi removido da UI)
    btnReload: document.getElementById('btnReloadProjects'),    // escondido
    btnContinue: document.getElementById('btnContinueProject'), // escondido
    structure: document.getElementById('projectStructure'),
    folderTree: document.getElementById('folderTree'),
    filtersBox: document.getElementById('filtersBox'),
    btnBack: document.getElementById('btnBackToProjects'),      // removido da UI
  ...
  function showProjectFlow(){
    ...
    if (projectFlow.btnContinue) projectFlow.btnContinue.disabled = !projectFlow.select.value;
  }
  ...
  // ===================== [TREEVIEW] renderização hierárquica =====================
  function renderFolderTree(list){
    projectFlow.folderTree.innerHTML = '';
    const arr = Array.isArray(list) ? list : [];
    if (!arr.length){
      projectFlow.folderTree.innerHTML = '<li style="color:#ccc; padding:.5rem;">Nenhuma pasta encontrada.</li>';
      return;
    }
    ...
    const build = (nodes) => {
      ...
      nodes.forEach(n => {
        ...
        item.className = 'tree-item' + ((n.children && n.children.length) ? '' : ' leaf');
        item.dataset.id = String(n.id ?? n.key ?? '');
        item.textContent = n.name || '(sem nome)';

        li.appendChild(item);

        if (n.children && n.children.length){
          const nested = build(n.children);
    ...
    // Abre nível raiz por padrão
    root.querySelectorAll(':scope > .tree-node > .tree-item').forEach((it) => {
      const nxt = it.nextElementSibling;
      if (nxt && nxt.classList.contains('nested')) {
        it.classList.add('expanded');
        nxt.classList.add('active');
      }
    });
    projectFlow.folderTree.appendChild(root);
  }

  // Delegação de clique para expandir/colapsar + seleção única
  if (projectFlow.folderTree) {
    projectFlow.folderTree.addEventListener('click', (ev) => {
      ...
      // Alterna expansão se tiver filhos
  ...
  // ====== seleção atual (apenas uma pasta) ======
  ...
  function updateApplyEnabled(){
    const hasFolder = !!document.querySelector('.tree-item.selected');
    const hasFilters = projectFlow.filtersBox.querySelector('input[type="checkbox"]:checked');
  ...
  // Se NÃO vier issueKey, alterna para o fluxo por projeto
  if (!issueKey) {
    showProjectFlow();
  ...
  /* ===== respostas do fluxo por Projeto ===== */
  ...
  if (projectFlow.select) projectFlow.select.addEventListener('change', () => {
    const key = projectFlow.select.value || '';
    if (!key) return;
    _projetoSelecionado = key;

...
exports.generateModelFromOpenAPI = void 0;
...
*/
function generateModelFromOpenAPI(openApiPath, endpoint, generatePathAppDriver) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (endpoint) {
            ...
            const className = (0, modelGenerator_1.generateClassNameFromPath)(endpoint);
            const methods = Object.keys(paths[endpoint] || {});
            if (methods.length === 0) {
                vscode.window.showErrorMessage(`Endpoint "${endpoint}" não encontrado no OpenAPI.`);
            }
            else {
                yield (0, modelGenerator_1.generateRequestBodyClasses)(endpoint, methods, paths[endpoint], openApiDoc, generatePathAppDriver);
            }
        }
        else {
            // Gera para todos os endpoints definidos em OpenAPI
            for (const endpointPath of Object.keys(paths)) {
                const className = (0, modelGenerator_1.generateClassNameFromPath)(endpointPath);
                const methods = Object.keys(paths[endpointPath]);
                yield (0, modelGenerator_1.generateRequestBodyClasses)(endpointPath, methods, paths[endpointPath], openApiDoc, generatePathAppDriver);
            }
        }
        vscode.window.showInformationMessage('Geração de Models concluída.');
    });
}
exports.generateModelFromOpenAPI = generateModelFromOpenAPI;
